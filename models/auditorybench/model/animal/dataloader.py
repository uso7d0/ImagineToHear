...
class QA_dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if any(audio_path.endswith(f'/sound_{i}.wav') for i in range(5)):
            in_audio = False
        else:
            in_audio = True
        ...
        results = find_span_token_position(query, tokenized_sentence, offsets, str(self.description[item]).lower())
        description_token = self.tokenizer.encode(self.description[item], add_special_tokens=False)
        if results['span_token_pos'] is None:
            span_token_pos = [0, 0]
        else:
            span_token_pos = [results["span_token_pos"], results["span_token_pos"] + len(description_token)]
        ...
        labels[~mask_token_index] = -100

        if len(input_ids) > 512:
            label = label[:512]

        return {"input_ids": input_ids, "attention_mask": attention_mask, "label": label, "audio": audio_array, "target":target, "span_token_pos": span_token_pos, "in_audio": in_audio}


def dynamic_padding_collate_fn(batch, extractor):
    max_seq_len = max([len(item["input_ids"]) for item in batch])
    max_target_len = 10
    input_ids, attention_mask, labels, audios, targets, span_token_pos, in_audio = [], [], [], [], [], [], []

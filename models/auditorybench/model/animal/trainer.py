...
def train_epoch(model, data_loader, optimizer, device, scheduler, fusion_idx):
    ...
    for step, d in enumerate(data_loader):
        ...
        outputs, predicts = model(
            audio_features=encoder_input_ids, span_token_pos=span_token_pos, input_ids=input_ids, targets=label, in_audio=in_audio, fusion_idx=fusion_idx, device=device
        )
        loss = outputs.loss
...
def validate(model, data_loader, fusion_idx, device):
    model = model.eval()
    losses = []
    outputs_arr = []
    cnt = 0

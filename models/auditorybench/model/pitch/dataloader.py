...
class QA_dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if any(audio_path_1.endswith(f'/sound_{i}.wav') for i in range(5)):
            in_audio1 = False
        else:
            in_audio1 = True
        ...
        if any(audio_path_2.endswith(f'/sound_{i}.wav') for i in range(5)):
            in_audio2 = False
        else:
            in_audio2 = True
        ...
        span1_token = self.tokenizer.encode(self.span1[item], add_special_tokens=False)
        if results["span1_token_pos"] is None:
            span1_token_pos = [0, 0]
        else:
            span1_token_pos = [results["span1_token_pos"], results["span1_token_pos"] + len(span1_token)]

        try:
            span2_token = self.tokenizer.encode(self.span2[item], add_special_tokens=False)
        except:
            span2_token = self.tokenizer.encode(" ", add_special_tokens=False)
            print("Error in span2_token")
        if results["span2_token_pos"] is None:
            span2_token_pos = [0, 0]
        else:
            span2_token_pos = [results["span2_token_pos"], results["span2_token_pos"] + len(span2_token)]
        ...
        return {
            ...
            "target": target,
            "in_audio": [in_audio1, in_audio2]
        }


def dynamic_padding_collate_fn(batch, extractor):
    max_seq_len = max([len(item["input_ids"]) for item in batch])
    input_ids, attention_mask, labels, first_token_pos, audios_1, audios_2, targets, in_audio = [], [], [], [], [], [], [], []
    ...
    return {
        ...
        'in_audio': torch.tensor(in_audio, dtype=torch.bool)
    }
...
def create_data_loader(df, tokenizer, extractor, batch_size, shuffle_=False):
    ds = QA_dataset(
        ...
        id_1_audio_path=df.audio_path1.to_numpy(),
        id_2_audio_path=df.audio_path2.to_numpy(),
        tokenizer=tokenizer,
    )
    return DataLoader(
        ds,

...
def parse_args():
    ...
    parser.add_argument("--lr", type=float, default=4e-5, required=False)
    ...
    parser.add_argument("--output_path", type=str, default="./audiobert/model/pitch_case/outputs", required=False)
...
def Train(args):
    ...
    wiki_df = pd.read_csv(f"./")

    train_df['audio_path1'] = train_df[f'audio_path1_{args.clap}']
    train_df['audio_path2'] = train_df[f'audio_path2_{args.clap}']
    dev_df['audio_path1'] = dev_df[f'audio_path1_{args.clap}']
    dev_df['audio_path2'] = dev_df[f'audio_path2_{args.clap}']
    test_df['audio_path1'] = test_df[f'audio_path1_{args.clap}']
    test_df['audio_path2'] = test_df[f'audio_path2_{args.clap}']
    wiki_df['audio_path1'] = wiki_df[f'audio_path1_{args.clap}']
    wiki_df['audio_path2'] = wiki_df[f'audio_path2_{args.clap}']
    ...
    wiki_data_loader = create_data_loader(wiki_df, tokenizer, extractor, args.batch_size)
    ...
    scheduler = get_cosine_schedule_with_warmup(
        optimizer, num_warmup_steps=int(total_steps * 0.1 * 20 / args.epochs), num_training_steps=total_steps
    )
    ...
    for epoch in range(args.epochs):
        ...
        dev_output, dev_loss, dev_acc = validate(model, dev_data_loader, device, args.fusion_idx-1)
        if dev_acc > max_acc:
            max_acc = dev_acc
    ...
    wiki_output, test_loss2, test_acc2 = validate(model, wiki_data_loader, args.fusion_idx-1, device)
